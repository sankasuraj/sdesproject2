...
class simpleapp_tk(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        label1 = Label(self, text="To Train a model out of a csv file click Train Model",
                       font=tkFont.Font(family="Helvetica", size=10))
        label1.grid(row=1, sticky=W)
        create = Tkinter.Button(self, text="Train Model",
                                command=self.Window2, height=1, width=10)
        create.grid(row=2, column=0, sticky='W', padx=5, pady=5)
        label2 = Label(self, text="To get the values using a model click Use Model",
                       font=tkFont.Font(family="Helvetica", size=10))
        label2.grid(row=3, sticky=W)
        use = Tkinter.Button(self, text="Use Model",
                             command=self.Window4, height=1, width=10)
    ...
    def OpenFile(self):
        ...
        if self.filename:
            self.window2.labelVariable.set(
                "Now save the model using Save button")
            # window2.save_model.config(state="normal")

    def OpenFile1(self):
        self.loadmodel = askopenfilename()
        if self.loadmodel:
            self.window4.labelVariable.set(
                "Now load the X data for which Y-values should be determined")

    def OpenFile2(self):
        self.xdata = askopenfilename()
        if self.xdata:
            self.window4.labelVariable.set(
                "Now save the name of the data using the Output button")

    def file_save(self):
        self.savemodel = asksaveasfile(mode='w', defaultextension=".csv")
        # asksaveasfile return `None` if dialog closed with "cancel".
        if self.savemodel is None:
            return
        if self.savemodel:
            self.window2.labelVariable.set(
                "Now press Next to start Training the data")
    ...
    def file_save1(self):
        ...
        if self.output:
            self.window4.labelVariable.set(
                "Now press Estimate to estimate the Y-values for given X-data")
    ...
    def Window2(self):
        ...
        self.window2.labelVariable.set(
            "Hello! Please choose a file using import button")

        import_data = Button(
            self.window2, text="Import Data", command=self.OpenFile)
        import_data.grid(row=1, column=0, rowspan=2, sticky='NSEW')

        save_model = Button(self.window2, text="Save Model",
                            command=self.file_save)
        save_model.grid(row=1, column=1, rowspan=2, sticky='NSEW')

        back = Button(self.window2, text="Back",
                      command=self.window2.destroy, height=1, width=10)
        back.grid(row=3, column=1, sticky='E', pady=5, padx=5)

        next = Button(self.window2, text="Next", command=combine_funcs(
            self.Window3, self.window2.destroy), height=1, width=10)
    ...
    def Window3(self):
        ...
        label4 = Label(self.window3, text="Progress Bar",
        ...
        back = Button(self.window3, text="Back", command=combine_funcs(
            self.window3.destroy, self.Window2))
        back.grid(row=2, column=1, sticky='E', pady=5, padx=5)

        next = Button(self.window3, text="Next", command=combine_funcs(
            self.Window4, self.window3.destroy))
    ...
    def Progress(self):
        ...
        self.window3.labelVariable.set(show)
        show += 'Esimated time remaining is approximately ' + \
    ...
    def Window4(self):
        ...
        self.window4.labelVariable.set(
            "Import the Loaded model Using 'Load Model' button")

        self.loadmodel = Button(
            self.window4, text="Load Model", command=self.OpenFile1)
        self.loadmodel.grid(row=1, column=0, rowspan=2)

        self.xdata = Button(self.window4, text="X Data",
                            command=self.OpenFile2)
        self.xdata.grid(row=1, column=1, rowspan=2)

        self.output = Button(self.window4, text="Output",
                             command=self.file_save1)
        self.output.grid(row=1, column=2, rowspan=2)

        estimate = Button(self.window4, text="Estimate",
                          command=self.find_values)
    ...
    def find_values(self):
        ...
        save = Button(self.window4, text="Next", command=combine_funcs(
            self.window4.destroy, self.Window3))
        save.grid(row=0, column=1, sticky='E', pady=5, padx=5)
        back = Button(self.window4, text="Back", command=combine_funcs(
            self.window4.destroy, self.Window3))
        back.grid(row=0, column=0, sticky='E', pady=5, padx=5)
        self.window4.labelVariable.set(
            "Estimated Y values, Now you may press Quit to quit the application")
