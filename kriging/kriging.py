...
class Solve:
	...
	def normx(self, x):
		...
		for i in range(self.k):
			normalised.append((x[i] - self.min_x[i]) / (self.max_x[i] - self.min_x[i]))
	...
	def neglikelihood(self):
		...
		self.SigmaSqr = ((self.y-self.mu).T.dot(np.linalg.solve(self.U, temp)))/self.n
...
def estimate_error(model, x, y):
	...
	error = np.sqrt(sum(np.ravel(estimate_y - y) * np.ravel(estimate_y - y))) / len(y)
	return error
